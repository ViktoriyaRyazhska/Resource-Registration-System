...
<c:if
  test="${not empty discreteParameters or not empty linearParameters}">
  <div>
    <h5>
      <i><spring:message code="label.search.message1" />:</i>
    </h5>
  </div>
</c:if>
<c:if test="${not empty discreteParameters}">
  <div id="discreteParameters" class="container">
    <c:forEach items="${discreteParameters}" var="discreteParameter">
      <div class="col-md-6 discreteParameter"
        ...
        </span> <label class="col-md-1">${discreteParameter.unitName}</label>
      </div>
    </c:forEach>
  </div>
</c:if>
  ...
  <div id="linearParameters" class="container">
    <c:forEach items="${linearParameters}" var="linearParameter">
      <div class="col-md-6 linearParameter" style="padding-bottom: 4px;"
        ...
        <span class="col-md-6 col-md-offset-2"> <input
          type="text" class="form-control value" />
        </span> <label class="col-md-1">${linearParameter.unitName}</label>
      </div>
    </c:forEach>
  </div>
</c:if>
